...
def upload_screenshot():
    from domains.ocr_evidence import extract_evidence_from_image, store_pending_evidence
    ...
    evidence_id = store_pending_evidence(clean_evidence)
...
@app.route("/api/confirm_dispute", methods=["POST"])
def confirm_dispute_endpoint():
    from domains.ocr_evidence import pop_pending_evidence
    ...
    evidence = pop_pending_evidence(evidence_id)
    if evidence is None:
        return jsonify({"error": "This evidence expired or was already filed — please re-upload the screenshot."}), 404

    result = raise_dispute_with_evidence(reason="Filed from uploaded screenshot", **evidence)
